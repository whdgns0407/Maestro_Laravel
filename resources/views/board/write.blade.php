...
<head>
    ...
    <title>{{ $board_category_sql->korean_name }} - 게시글 작성</title>
...
<body>
    <table class="table">
        <tr>
            <td colspan="2" style="text-align:center;">{{ $board_category_sql->korean_name }} - 게시글작성</td>
        </tr>
        <tr>
                    ...
                    <select class="form-control" id="category" name="category">
                        @foreach ($board_title_catetories as $board_title_category)
                            <option value="{{ $board_title_category->id }}">
                                {{ $board_title_category->title_name }}</option>
                        @endforeach
                    </select>
                </td>
                @php
            ...
            <td style="width:80%;" colspan="{{ $title_colspan }}">
                <input type="text" class="form-control" id="title" name="title" style="width:100%;"
                    placeholder="제목을 입력하여주세요." required>
            </td>

        </tr>
        <tr>
            <td colspan="2">
                <textarea id="content" name="content" rows="20" style="width:100%;" required></textarea>
            </td>
        </tr>
        <tr>
            <td colspan="2">
        ...
        $(document).ready(function() {
            ...
            $("#write_post").click(function() {
                ...
                $.ajax({
                    ...
                    url: "{{ route('board_write_post', ['board_name' => $board_name]) }}", // 실제 서버 엔드포인트 URL
                    data: {
                        category: category,
                        title: title,
                        content: content,
